...
{
	class LIBSERIALIZER_API IColumnsDefinition
	{
	public:
		enum ETypes
		{
	...
	public:
		...
		virtual size_t AddColumn(const string::TString& strColumnName, ETypes eColType) = 0;
		virtual void Clear() = 0;

		virtual size_t GetColumnIndex(const wchar_t* strColumnName) = 0;
		virtual const string::TString& GetColumnName(size_t stIndex) const = 0;
	...
	template<> struct ColumnType<string::TString> { static const IColumnsDefinition::ETypes value = IColumnsDefinition::eType_string; };
	template<> struct ColumnType<chcore::TSmartPath> { static const IColumnsDefinition::ETypes value = IColumnsDefinition::eType_path; };
