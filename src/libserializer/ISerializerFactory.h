...
{
	class LIBSERIALIZER_API ISerializerFactory
	{
	public:
		virtual ~ISerializerFactory();

		virtual ISerializerPtr CreateTaskManagerSerializer(bool bForceRecreate = false) = 0;
		virtual ISerializerPtr CreateTaskSerializer(const logger::TLogFileDataPtr& spLogFileData, const string::TString& strNameHint = _T(""), bool bForceRecreate = false) = 0;
	};
