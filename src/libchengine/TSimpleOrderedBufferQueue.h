	...
	public:
		...
		void Push(TOverlappedDataBuffer* pBuffer)
		{
			...
			if(!insert(pBuffer).second)
				throw chcore::TCoreException(chcore::eErr_InvalidArgument, L"Buffer already exists in the collection", LOCATION);
			m_spCount->Increase();
		}
		...
		{
			if(!spBuffers)
				throw chcore::TCoreException(chcore::eErr_InvalidArgument, L"spBuffers is NULL", LOCATION);
