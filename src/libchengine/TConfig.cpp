...
namespace chengine
{
	...
	bool TConfig::GetValue(PCTSTR pszPropName, TPathContainer& rvValues) const
	{
		rvValues.Clear();

		TStringArray arrStrings;
		bool bResult = GetImpl()->GetArrayValueNoDefault(pszPropName, arrStrings);

		if(bResult)
		{
			TStringArray vPaths;
			if(GetValue(pszPropName, vPaths))
			{
				for(size_t stIndex = 0; stIndex < vPaths.GetCount(); ++stIndex)
				{
					rvValues.Add(PathFromWString(vPaths.GetAt(stIndex)));
				}
				return true;
			}
	...
	TConfig& TConfig::SetValue(PCTSTR pszPropName, const TPathContainer& rvValues)
	{
		TStringArray vPaths;
		...
		{
			const TSmartPath& spPath = rvValues.GetAt(stIndex);
			vPaths.Add(spPath.ToWString());
		}

		SetValue(pszPropName, vPaths);

		if (GetImpl()->SetArrayValue(pszPropName, vPaths))
			SendNotification(pszPropName);
