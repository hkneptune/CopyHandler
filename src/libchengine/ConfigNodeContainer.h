...
namespace chengine
{
	namespace details
	{
		struct ChangeValue
		{
			explicit ChangeValue(const string::TString& strNewValue);
		...
		private:
			string::TString m_strNewValue;
			bool m_bWasModified = false;
		};

		struct ChangeOrderAndValue
		{
			ChangeOrderAndValue(const string::TString& tNewValue, int iOrder);
		...
		public:
			...
			void AddEntry(PCTSTR pszPropName, int iIndex, const string::TString& strValue);
			...
			template<>
			string::TString GetValue<string::TString>(PCTSTR pszPropName, const string::TString& rDefaultValue) const
			{
				boost::shared_lock<boost::shared_mutex> lock(m_lock);
				string::TString tResult = rDefaultValue;
			...
			template<>
			bool GetValueNoDefault<string::TString>(PCTSTR pszPropName, string::TString& rValue) const
			{
				boost::shared_lock<boost::shared_mutex> lock(m_lock);
				ConfigNodeContainer::NodeContainer::const_iterator iterFnd = m_mic.find(boost::make_tuple(pszPropName, 0));
				if (iterFnd != m_mic.end())
			...
			bool GetValueNoDefault<bool>(PCTSTR pszPropName, bool& rValue) const
			{
				...
				if (iterFnd != m_mic.end())
				{
					const string::TString& strValue = (*iterFnd).m_strValue.Get();
			...
			// vector-based values
			string::TStringArray GetArrayValue(PCTSTR pszPropName, const string::TStringArray& rDefaultValue) const;
			bool GetArrayValueNoDefault(PCTSTR pszPropName, string::TStringArray& rValue) const;
			bool SetArrayValue(PCTSTR pszPropName, const string::TStringArray& rValue);
		...
		private:
			void ImportNode(string::TString strCurrentPath, const boost::property_tree::wiptree& rTree);
		...
		public:
			typedef boost::multi_index_container<ConfigNode,
				...
				boost::multi_index::const_mem_fun<ConfigNode, string::TString, &ConfigNode::GetNodeName>,
			...
			string::TString m_strFilePath;

			boost::signals2::signal<void(const string::TStringSet&)> m_notifier;
			string::TStringSet m_setDelayedNotifications;
			bool m_bDelayedEnabled;
			serializer::object_id_t m_oidLastObjectID;

			serializer::TRemovedObjects m_setRemovedObjects;
