...
namespace string
{
		...
		m_stBufferSize(0)
	{
		...
		if (pszEnd != nullptr && pszStart == nullptr || pszEnd == nullptr && pszStart != nullptr)
			throw TStringException("End of string specified while start is nullptr");

		// sanity check
		if (pszEnd < pszStart)
			throw TStringException("Paradox: string begins after its end");

		size_t stCount = pszEnd - pszStart;
		if (stCount > stMaxStringSize)
			throw TStringException("Exceeded maximum expected string size");
	...
	{
		if (!pszStart)
			throw TStringException("String not specified");
	...
	bool TString::Delete(size_t stIndex, size_t stCount)
	{
		...
		if (err != 0)
			throw TStringException("Failed to move memory");
...
string::TString operator+(const wchar_t* pszString, const string::TString& str)
{
	string::TString strNew(pszString);
	strNew += str;
	return strNew;
}
